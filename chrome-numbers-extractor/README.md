...
- **ברירות מחדל:**
  ...
  - ערכים: **טקסט מקורי** (כל המספרים, בדיוק כפי שהם בדף).
  - היקף: **הכול** (כל המספרים בדף).
...
- **לחיצה אחת:** כשמשאירים את ברירות המחדל, פותחים את התוסף ולוחצים **"הרץ עכשיו"** — זהו.
  התצוגה המקדימה והבחירה הן אופציונליות בלבד, למי שרוצה לסנן לפני הכתיבה.
- ריצה **חד‑פעמית** (כפתור "הרץ עכשיו") או **חוזרת עם טיימר**.
- **"כמו ההרצה האחרונה":** כפתור שמופיע אחרי הרצה מוצלחת ומריץ שוב, בלחיצה אחת,
  עם אותן הגדרות של הריצה הקודמת (יעד/עמודה/מצב ערכים) — גם אם שינית משהו בטופס בינתיים.
- **תצוגה מקדימה + בחירה:** כפתור "תצוגה מקדימה" סורק את הדף ומציג טבלה של כל המספרים
  (ערך מקורי מול ערך מנוקה) **בלי לכתוב לשום מקום**. שורות שבהן הניקוי שינה את הערך מסומנות
  בכחול, ומספרים לא תקינים באדום. לכל שורה יש תיבת סימון (וגם "סמן/נקה הכול"), וכפתור
  **"כתוב נבחרים"** כותב לגיליון בדיוק את המספרים שסימנת, לפי הסדר — כך אפשר לסנן
  למשל טלפונים או מספרים לא רצויים לפני הכתיבה.
